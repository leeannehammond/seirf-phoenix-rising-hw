...
- Do a query specific to the custom bounty you inserted
...
- Find the bounty with a ship name of `Waverider` OR `Serenity`
- Find the bounty who is not captured AND whose client is `Ayesha High Priestess of the Sovereign`
- Increase all the bounties by 333333
- Multiply all the bounties by 2
- Add `Bobba Fett` as a hunter for `Malcolm Reynolds`
- Add `Bobba Fett` as a hunter for the document that has the ship `Waverider`
- Find and remove `Dengar` the bounty hunter
- Upserts will insert a value if it doesn't exist, if it does exist it will update it
  - Try giving a new field of `lastSeen` to Han Solo, with the property `yesterday` and set `upsert` to `true`
